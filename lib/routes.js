...
router.get('/i/:gallery/:page', ctx => {
  ...
  const { w, h = 'auto', fit = 'cover' } = ctx.request.query
  const width = parseInt(w)
  const height = parseInt(h)
  if (!gallery || !page || !parseInt(page) || !width || width > 2048) {
    ctx.status = 400
    return
  }

  if (h !== 'auto' && (!height || height > 2048)) {
    ctx.status = 400
    return
  }
  ...
  const cachedFile = `./imgcache/${gallery}-${page}-${width}${fileHeight}${fileFit}.webp`
